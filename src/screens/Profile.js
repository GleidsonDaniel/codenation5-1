...
export default class Profile extends React.PureComponent {
  ...
  getProfile = async () => {
    ...
    const USER_TOKEN =  JSON.parse(result.token)
    const AuthStr = USER_TOKEN
    
    let getItemResponse = await axios.get('https://api.codenation.dev/v1/acceleration', { headers: { Authorization:  '123'  }})

    if (getItemResponse){
        try {
          this.setState({ profile: getItemResponse })
        } catch (error) {
